...
### Evidencia de Datos Desnormalizados
![Captura de registros sucios Hospitales](diagramas_er/imagen_hospitales.jpg)
...
### Evidencia de Datos Desnormalizados
![Captura de registros sucios Netflix](diagramas_er/imagen_netflix.jpg)
...
### Justificación de Violación a Formas Normales
Este documento presenta anomalías severas que rompen los principios de las tres primeras formas normales, impidiendo la escalabilidad, eficiencia e integridad de la base de datos:

* **Violación a la Primera Forma Normal (1FN) - Falta de Atomicidad:** La regla cardinal de la 1FN exige que cada celda contenga un valor único y atómico. En este dataset, las columnas `cast`, `director`, `country` y `listed_in` contienen arreglos de texto separados por comas. Al carecer de atomicidad, el motor de base de datos no puede crear índices eficientes; buscar todas las películas de un actor específico obligaría al sistema a realizar un escaneo completo de texto (`LIKE '%Actor%'`) fila por fila. Además, esta estructura imposibilita realizar operaciones relacionales básicas como agrupaciones (`GROUP BY`) o uniones (`JOIN`).

* **Violación a la Segunda Forma Normal (2FN) - Dependencias Parciales Latentes:** La violación a la 2FN se hace evidente al observar la redundancia que se generaría si intentáramos cumplir la 1FN sin separar las tablas. Si decidiéramos "aplanar" los datos creando un registro nuevo por cada actor de una película (formando una llave primaria compuesta `show_id` + `actor`), la estructura violaría inmediatamente la 2FN. Esto se debe a que atributos como `title`, `release_year`, `duration` o `description` dependerían parcial y exclusivamente del `show_id`, y no del actor. El resultado sería la repetición masiva de toda la información de la película tantas veces como miembros del elenco tenga.

* **Violación a la Tercera Forma Normal (3FN) - Mezcla de Entidades y Anomalías de Actualización:** La 3FN exige que no existan dependencias transitivas y que cada entidad lógica resida en su propia estructura. Al incrustar cadenas de texto con nombres de directores, actores, géneros y países directamente dentro de la entidad `shows`, se están mezclando dominios. Esto provoca anomalías de actualización graves: si el nombre de un país cambia o un director necesita una corrección ortográfica en su nombre, el sistema tendría que buscar y sobreescribir cientos de cadenas de texto dispersas en la tabla transaccional, en lugar de actualizar una única celda en un catálogo maestro referenciado por una llave foránea (FK).
